    ...
    if [ "\$ss_tag" == "st" ]; then
        ...
        if [ "\$tu_show" == "true" ]; then
            ...
            if command -v ss &>/dev/null; then
                ...
                tcp_connections=\$(netstat -at | grep '^tcp' | grep -v 'LISTEN' | sed -e 's/\(::ffff:\)\?//g' | grep -v '0.0.0.0:*' | grep -v '\[::\]:*' | grep -v ':::*')
                tut_tool="netstat"
                ip_location=5
            else
            ...
            if command -v ss &>/dev/null; then
                ...
                udp_connections=\$(netstat -au | grep '^udp' | grep -v 'LISTEN' | sed -e 's/\(::ffff:\)\?//g' | grep -v '0.0.0.0:*' | grep -v '\[::\]:*' | grep -v ':::*')
                tuu_tool="netstat"
                ip_location=5
            else
